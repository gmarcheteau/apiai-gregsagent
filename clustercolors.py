...
def fitColorClustering(image_resized):
  ...
  bestClusters = 0;
  MIN_CLUSTERS = 3
  MAX_CLUSTERS = 6 #max number of different "colors" to identify
  silscores = []
  #threshold for silhouette score: increase number of clusters if not reached
  SILHOUETTE_THRESHOLD = 0.88
  silhouette = 0
  number_clusters = MIN_CLUSTERS
  
  while (silhouette<SILHOUETTE_THRESHOLD) and (number_clusters<MAX_CLUSTERS+1):
      # Cluster colours
      clt = KMeans(n_clusters = number_clusters,n_init = 15,precompute_distances = True,init="k-means++")
      #clt = MiniBatchKMeans(n_clusters = number_clusters,n_init = 15)
      #clt = KMeans(n_clusters = number_clusters)
      clt.fit(image_array)
      ...
      print "With",number_clusters,"clusters:"
